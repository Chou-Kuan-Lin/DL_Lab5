...
int main(int argc, const char *argv[]) {
  ...
  tdl.load("Lab5_weight");
  std::ofstream outfile ("Lab5_result.txt");
  ...
  for (size_t n = 1; n <= total; n++) {
    ...
    thisEpochMax = tdl.make_statistic(n, b, score);

    path.clear();

    if( trainEpochMax < thisEpochMax )
    {
        std::cout << "should save " << thisEpochMax << " > " << trainEpochMax << std::endl;
        trainEpochMax = thisEpochMax;
        tdl.save("Lab5_weight");
    }
	
	if( n%1000==0 )
        outfile << thisEpochMax << std::endl;
  }
